...
class DisposalAssetController extends Controller
{
        ...
        ->get(['mutations.id as id', 'users.full_name as name',DB::raw('DATE_FORMAT(mutations.created_at, "%d-%b-%Y") as created_ats'),
          ...
          ->addColumn('action', function($row){
                $btn =  '<a href="" data-toggle="modal" onclick=getDetailMutation('.$row['id'].') data-target="#exampleModal" class="edit btn btn-info btn-sm">Detail Mutation</a>&nbsp;'.
                        '<a href="/disposal_asset/ba_disposal_asset/'.$row['id'].'")" class="btnPrints btn btn-warning btn-sm" id=""><i class=\'fa fa-print\'></i>Print</a>&nbsp'.
                        '<a href="/disposal_asset_excel/'.$row['id'].'")" class=" btn btn-success btn-sm" id=""><i class=\'fa fa-file-excel\'></i> Excel</a> &nbsp;';
                        return $btn;
          })
          ->rawColumns(['action'])
          ->make(true);
    ...
    public function disposal_asset_excel($id)
    {
      $get_disposal = json_decode(Mutations::select(DB::raw('DATE_FORMAT(mutations.created_at, "%d-%b-%Y") as created_ats'))->where('id', $id)->first());
      return Excel::download(new DisposalAsset($id), "exportDisposalAsset_".$get_disposal->created_ats.".xlsx");
    }
